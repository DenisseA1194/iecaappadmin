...
struct ProgramaView: View {
    ...
    @State var seleccionPrograma: Int = 0
    @StateObject var viewModelPrograma = CatalogoCursosProgramasViewModel()
    ...
    var body: some View {
        NavigationView {
            Form {
                Section(header: Text("Selecciona un programa")) {
                    Picker("Programas", selection: $seleccionPrograma) {
                        
                        ForEach(viewModelPrograma.catalogoCursosProgramas.indices.map { $0}, id: \.self) { index in
                            Text(viewModelPrograma.catalogoCursosProgramas[index].Nombre)
                                .tag(index)
                        }
                    }.onChange(of: seleccionPrograma) { index in
                        print(index)
                        if index != 0 {
                            let zonaSeleccionada = viewModelPrograma.catalogoCursosProgramas[index]
                        ...
                        viewModelPrograma.fetchCatalogoCursosProgramas()
                        ...
                        ToolbarItem(placement: .navigationBarTrailing) {
                            Button(action: {
                                ...
                                Alert(title: Text("Aviso"), message: Text("Agrega una nuevo programa"), dismissButton: .default(Text("Aceptar")))
                            }
                        }
                    }
                }
                    ...
                    Button(action: {
                        if todosLosCamposEstanCompletos() {
                            if buttonText == "Agregar"{
                                ...
                                let nuevoPrograma = CatalogoCursosProgramas(
                                    Id:"",
                                ...
                                viewModelPrograma.agregarNuevoCatalogoCursosProgramas(catalogoCursosProgramas: nuevoPrograma)
                                viewModelPrograma.fetchCatalogoCursosProgramas()
                            }else{
                                avisoText = "Registro Modificado"
                                showAlertAgregarModificar.toggle()
                                let programa = CatalogoCursosProgramas(
                                    Id:_Id,
                                    IdEmpresa: "4BBC69B0-F299-4033-933F-2DE7DC8B9E8C",
                                    Fecha: "2024-01-30T19:06:05.675Z",
                                    Nombre: _Nombre,
                                ...
                                viewModelPrograma.editarCatalogoCursosProgramas(catalogoCursosProgramas: programa)
                                viewModelPrograma.fetchCatalogoCursosProgramas()
                            }
                        }else{
