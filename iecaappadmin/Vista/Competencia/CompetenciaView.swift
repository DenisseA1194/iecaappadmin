...
struct CompetenciaView: View {
    @State private var newRazonSocialName = ""
    @State private var newRepresentante = ""
    @State private var notas = ""
    
    
    @State var seleccionCompetencia: Int = 0
    @StateObject var viewModelCompetencia = CompetenciaViewModel()
    ...
    var body: some View {
        NavigationView {
            Form {
                Section(header: Text("Selecciona una competencia")) {
                    Picker("Competencias", selection: $seleccionCompetencia) {
                        
//                        ForEach(viewModelCompetencia.competencias.indices.map { $0}, id: \.self) { index in
//                            Text(viewModelCompetencia.competencias[index].Nombre)
//                                .tag(index)
//                        }
                        let i = 0
                        ForEach([0] + viewModelCompetencia.competencias.indices.map { $0 + 1 }, id: \.self) { index in
                            if index == 0 {
                                Text("")
                                    .tag(0)
                            } else {
                                Text(viewModelCompetencia.competencias[index - 1].Nombre)
                                    .tag(index)
                            }
                        }
                    }.onChange(of: seleccionCompetencia) { index in
                        print(index)
                        if index != 0 {
                            let zonaSeleccionada = viewModelCompetencia.competencias[index-1]
                        ...
                        viewModelCompetencia.fetchCompetencias()
                        ...
                        ToolbarItem(placement: .navigationBarTrailing) {
                            Button(action: {
                                ...
                                Alert(title: Text("Aviso"), message: Text("Agrega una nueva competencia"), dismissButton: .default(Text("Aceptar")))
                            }
                        }
                    }
                }
                    ...
                    Button(action: {
                        if todosLosCamposEstanCompletos() {
                            if buttonText == "Agregar"{
                                ...
                                let nuevaCompetencia = Competencia(
                                    Id:"",
                                ...
                                viewModelCompetencia.agregarNuevaCompetencia(nuevaCompetencia: nuevaCompetencia)
                                viewModelCompetencia.fetchCompetencias()                            }else{
                                    ...
                                    viewModelCompetencia.editarCompetencia(competencia: competencia)
                                    viewModelCompetencia.fetchCompetencias()
                                }
                        }else{
